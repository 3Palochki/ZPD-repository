composers = [
  {
    "status": {
      ...
      "source": "db",
      "rows": 220,
      "processingtime": 0.0008740425109863281,
      "api": "Open Opus-dyn"
    },
    "composers": [
      {
...
// Display a new random composer
function displayNewComposer() {
  // Select a random composer from the array
  const randomIndex = Math.floor(Math.random() * composers[0].composers.length);
  const composer = composers[0].composers[randomIndex];

  // Display the composer's portrait
  portrait.src = composer.portrait;

  // Shuffle the composers array and select 3 random options
  const shuffledComposers = shuffle(composers[0].composers);
  const randomOptions = shuffledComposers.slice(0, 3);

  // Add the correct option to the array
  randomOptions.push(composer);

  // Shuffle the options
  const shuffledOptions = shuffle(randomOptions);

  // Create the HTML for the options
  let optionsHTML = "";
  shuffledOptions.forEach((option) => {
    optionsHTML += `
      <button class="option" onclick="checkAnswer('${option.complete_name}', '${composer.complete_name}')">
...
function checkAnswer(selectedOption, correctOption) {
  if (selectedOption === correctOption) {
    ...
    resultBox.innerHTML = "Incorrect. The correct answer is " + correctOption + ".";
  }
}
